...
func _ready():
	...
	# Crear timer para actualizar el mercado cada 5 minutos
	create_update_timer()
	
	# Crear timer para mostrar countdown
	create_countdown_timer()
	
	print("TransferMarket: Inicialización completada")

func _on_back_pressed():
	print("TransferablePlayersMarket: Volviendo al menú principal del mercado")
...
# Timer para actualizar el mercado cada 5 minutos
func create_update_timer():
	if TransferMarketManager:
		# Conectar señal de actualización del mercado
		TransferMarketManager.market_updated.connect(_on_market_updated)

func _on_market_updated():
	print("TransferablePlayersMarket: Mercado actualizado - refrescando jugadores")
	update_players()

# Timer para mostrar countdown
func create_countdown_timer():
	var countdown_timer = Timer.new()
	countdown_timer.set_wait_time(1.0)
	countdown_timer.process_mode = Node.PROCESS_MODE_ALWAYS
...
func _update_countdown():
	if TransferMarketManager:
		...
		var time_since_update = (Time.get_ticks_msec() / 1000.0) - TransferMarketManager.market_last_update
		var time_remaining = TransferMarketManager.market_update_interval - time_since_update
		
		if time_remaining <= 0:
			time_remaining = 0
		
		# Actualizar título con countdown
		var title_label = get_node_or_null("VBoxContainer/Title")
		if title_label:
			var minutes = int(time_remaining) / 60
			var seconds = int(time_remaining) % 60
			title_label.text = "JUGADORES TRANSFERIBLES\n(Próxima actualización: %02d:%02d)" % [minutes, seconds]
